...
def register() -> None:
    register_tool(
        ...
        description=(
            ...
            "not 'Index'. "
            "IMAGE QUALITY — tell the user when delivering the image: the stored "
            "alert image is a LOW-RES thumbnail (and 'markup'/AI box won't "
            "render) UNLESS the camera has hi-res alert JPEGs enabled (BI camera "
            "settings → Alerts tab; registry `hiresalerts=1`). This is a "
            "camera-side setting — the /alerts/ endpoint can only serve what BI "
            "stored and ignores w/h/scale params, so the tool cannot upscale it. "
            "If the user wants full-resolution alert stills with AI markup, they "
            "must enable that toggle (storage-cost impact; affects future alerts "
            "only). State this proactively rather than waiting to be asked."
        ),
        schema={
            "type": "object",
            "properties": {
